...
const deadline = new Date("Thu Sep 09 2021 5:15:00 GMT-0700 (Pacific Daylight Time)");
const now = new Date()
...
const MakePicks = () => {
    ...
    const uploadPicks = async () => {
      // db.collection("week1").doc(currentUser.displayName).set(games);
      // db.collection(currentUser.displayName).doc("week1").set({picks: games});
		console.log(currentUser.uid);
      db.collection("week1").doc(currentUser.uid).set({name: currentUser.displayName, picks: games});
    }
    ...
    return (
        <>
          <Card>
            <Card.Body>
            <h2 className="text-center mb-4">Make Picks</h2>
          ...
          <Link to={{pathname: "/standings", thisWeekGames: {thisWeekGames}}} className="btn btn-success w-100 mt-3">
                Press to View Standings
            </Link>
          <Table className="submittedTable" striped bordered hover>
            <thead>
              <tr>
                <th>Away</th>
                <th>Home</th>
